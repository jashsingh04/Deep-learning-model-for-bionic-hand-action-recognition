...
reduce_lr_on_plateau = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=2)
...
def preprocess_data(sensor_number):
    ...
    if int(sensor_number) == 1:
        ...
        dataset_features = dataset_features_sensor.iloc[:, 1000:2000]
    else:
        dataset_features = dataset_features_sensor.iloc[:, 2000:3000]
...
def preprocess_data_multi(sensor1, sensor2):
    ...
    if sensor1 == 1 and sensor2 == 2:
        dataset_features = dataset_features_sensor.iloc[:, :2000]
    elif sensor1 == 1 and sensor2 == 3:
        first_slice = dataset_features_sensor.iloc[:, :1000]
        last_slice = dataset_features_sensor.iloc[:, -1000:]
        dataset_features = pd.concat([first_slice, last_slice], axis=1)
        print(dataset_features.shape)
    else:
        dataset_features = dataset_features_sensor.iloc[:, 1000:3000]
...
def get_all_emg_data():
    ...
    dataset_labels = dataset['label']
    dataset_features_sensor = dataset.drop(columns=['label'], axis=1)
    dataset_features = dataset_features_sensor.iloc[:, :3000]
    print(dataset_features)
    sensor_dataset = dataset_features + dataset_labels
    encoder = LabelEncoder()
    encoder.fit(dataset_labels)
...
def train_data():
    ...
    try:
        ...
        for train, test in kfold.split(x, y):
            ...
            history = model.fit(x[train], y[train], epochs=100, batch_size=100, validation_data=(x[test], y[test]),
                                )
...
# this function is used to load the model and convert it into tflite model.
def load_cmodel():
    global cmodel
    try:
...
# This function is calculating the accuracy of the model that we have converted into tflite

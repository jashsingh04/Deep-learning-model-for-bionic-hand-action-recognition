...
#     # Customize the feature configuration as needed
#     cfg = tsfel.get_features_by_domain()
#     # Extract features from the EMG data
#     extracted_features = tsfel.time_series_features_extractor(cfg, emg_data)
#     # Convert the extracted_features DataFrame to a NumPy array
#     extracted_features = extracted_features.to_numpy()
#     print("Shape of emg_data:", emg_data.shape)
#     print("Data type of emg_data:", emg_data.dtype)
#     print("Shape of extracted_features:", extracted_features.shape)
#     print("Data type of extracted_features:", extracted_features.dtype)
#
...
def extract_features(emg_data):
    ...
    # Initialize an empty list to store extracted features for each sample
    extracted_features_list = []

    # Iterate through each sample in emg_data and extract features
    for sample in emg_data:
        ...
        # Convert the sample_features DataFrame to a NumPy array
        sample_features = sample_features.to_numpy()
        extracted_features_list.append(sample_features)
        count+=1

    # Concatenate the list of sample features into a NumPy array
...
def import_data():
    ...
    try:
        ...
        label_info_label.config(text=f"Dataset Imported Successfully: {filename}")
    except Exception as e:
        display_error(f"Error occurred while importing data: {e}")


def preprocess_data(sensor_number):
    global dataset, x, y, sensor_dataset
    dataset_labels = dataset['label']
    dataset_features_sensor = dataset.drop(columns=['label'], axis=1)
    if int(sensor_number) == 1:
        dataset_features = dataset_features_sensor.iloc[:, :500]
    elif sensor_number == 2:
        dataset_features = dataset_features_sensor.iloc[:, 500:1000]
    else:
        dataset_features = dataset_features_sensor.iloc[:, 1000:1500]

    sensor_dataset = dataset_features + dataset_labels
...
def get_emg_data(sensor_name):
    ...
    if sensor_name == "EMG1":
        ...
        preprocess_data(3)
    sensor_info_label.config(text=f"Sensor: {sensor_name} data preprocessing completed")


def preprocess_data_multi(sensor1, sensor2):
    global x, y, dataset, sensor_dataset
    dataset_labels = dataset['label']
    dataset_features_sensor = dataset.drop(columns=['label'], axis=1)
    if sensor1 == 1 and sensor2 == 2:
        dataset_features = dataset_features_sensor.iloc[:, :1000]
    elif sensor1 == 1 and sensor2 == 3:
        first_slice = dataset_features_sensor.iloc[:, :500]
        last_slice = dataset_features_sensor.iloc[:, -500:]
        dataset_features = pd.concat([first_slice, last_slice], axis=1)
...
def get_multi_emg_data(sensor1, sensor2):
    if sensor1 == "EMG1" and sensor2 == "EMG2":
        preprocess_data_multi(1, 2)
    elif sensor1 == "EMG1" and sensor2 == "EMG3":
        preprocess_data_multi(1, 3)
    else:
        preprocess_data_multi(2, 3)

    sensor_info_label.config(text=f"Sensor: {sensor1} and {sensor2} data preprocessing completed")


def get_all_emg_data():
    global x, y, dataset, sensor_dataset
    dataset_labels = dataset['label']
    dataset_features_sensor = dataset.drop(columns=['label'], axis=1)
    dataset_features = dataset_features_sensor.iloc[:, :1500]
    print(dataset_features)
    sensor_dataset = dataset_features + dataset_labels
    encoder = LabelEncoder()
    encoder.fit(dataset_labels)
    ...
    print(dataset_labels.unique())
    print(y)
    x = np.array(dataset_features[:])
    x = x.reshape(x.shape[0], x.shape[1], 1)
    x = np.asarray(x).astype(np.float32)
    sensor_info_label.config(text=f"preprocessed dataset for all sensors")
...
def on_select_option(*args):
    ...
    elif selected_option == "Only EMG2 and EMG3":
        get_multi_emg_data('EMG2', 'EMG3')
    elif selected_option == "Only EMG1 and EMG3":
        get_multi_emg_data('EMG1', 'EMG3')
    elif selected_option == "All EMG":
        get_all_emg_data()
        pass
    else:
        pass
...
def extract_and_train_features():
    global x, y

    if user_model_name == '':
        display_error("You must provide the user model name before training the model")
        return

    try:
        x = x.reshape(x.shape[0], -1)
        extracted_features = extract_features(x)# Modify to extract from the selected data source
        ...
        for train, test in kfold.split(extracted_features, y):
...
Action_menu = OptionMenu(root, click_action, "Only EMG1", "Only EMG2", "Only EMG3", "Only EMG1 and EMG2",
                         "Only EMG2 and EMG3", "Only EMG1 and EMG3", "All EMG")
...
label_info_label = Label(label_info_frame, text="File info", font=("Arial", 9, "bold"), wraplength=200)
...
Train_button = Button(root, text="Train Data", height=2, width=60, command=extract_and_train_features)

...
def accept_data():
    ...
    if not serial_thread or not serial_thread.is_alive():
        ...
        test_value=list(data1.queue)+ list(data2.queue)
...
def live(data):
    ...
    # add model location
    interpreter = tf.lite.Interpreter(model_path="jash_emg12.tflite")
    ...
    # Data from the sensor must come here
    data1 = data

    # Helper function to process a set of 1000 values through the TFLite model
    def process_data(input_data):
        input_data = np.array([data1], dtype=np.float32)
    ...
    predicted_label_index = np.argmax(predictions_data1)

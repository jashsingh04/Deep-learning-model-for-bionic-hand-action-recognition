...
def train_data():
    ...
    for train, test in kfold.split(x, y):
        ...
        history = model.fit(x[train], y[train], epochs=total_epochs, batch_size=100, verbose=0,
        ...
        def process_epoch(epoch):
            ...
            return model.fit(x[train], y[train], epochs=epoch + 1, initial_epoch=epoch, batch_size=100, verbose=0,
                             validation_data=(x[test], y[test]))

        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(process_epoch, epoch) for epoch in range(fold_iterations)]
            for future in concurrent.futures.as_completed(futures):
                iteration += 1
                progress_value = int(iteration * 100 / total_iterations)
                progress_bar['value'] = progress_value
                progress_bar.update()
                future.result()  # Retrieve the result of each epoch
...
label_info_frame.place(x=30, y=70)
